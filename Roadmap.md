...
- [ ] Lighthouse ≥ 90 (mobile) on every page — **except the legal pages, by design.** `/privacy` scores Performance 95 / Accessibility 95 / Best Practices 100 / **SEO 58**. The SEO number is caused solely by the intended `noindex, follow`: Lighthouse's `is-crawlable` audit fails on any noindexed page. Measured directly — removing the tag scores 92. Keeping legal pages out of search is the correct trade; do not "fix" this by dropping noindex.
