...
## Phase 3 — Launch pass
- [x] **Prerendering done — React Router framework mode, `ssr: false` + `prerender`** (15 Aug 2026). All 13 routes emit their own static HTML; `/services` and an unknown path are no longer byte-identical (54,780B vs 18,168B). Canonical, 5 OG, 4 Twitter, JSON-LD and one `<h1>` are in the raw HTML with JS disabled on every route. Unknown paths return a real **404** with the branded page, via `build/client/404.html` emitted by `scripts/emit-404.mjs` from the prerendered splat route. Zero hydration errors across 13 routes x 2 themes, deployed. `appDirectory: "src"` keeps the existing tree; `index.html`, `main.tsx` and `App.tsx` are deleted.
- [x] **Route-level code-splitting — CLAUDE.md Section 10 gap closed.** 1 chunk (396KB) became 32. First load 361–380KB raw depending on route; a client-side navigation now fetches only the next route's chunk (measured: 19KB for `/` → `/services`).
- [x] **Lighthouse SEO 100 on every route measured** (was 92), Accessibility 100, Best Practices 100, TBT 0ms. Performance on a gzip-serving production build: `/` 90, `/portfolio/sangira` 93, `/services` 95, `/contact` 96. **Measure with compression on** — an uncompressed static server scores `/` at 88 because prerendered HTML is ~32KB raw vs the old 1,968B shell; Vercel serves brotli.
- [ ] **`/dev/components` is prerendered and therefore publicly reachable.** It had to be enumerated: with the SPA catch-all gone, an un-enumerated route hard-404s. When removing it, delete the route in `src/routes.ts`, the page, and its entry in `react-router.config.ts` together.
