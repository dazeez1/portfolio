...
- [ ] **`theme-color` disagrees with the page in 3 of 4 states — owner decision.** The two `prefers-color-scheme` meta tags in index.html tint mobile browser chrome, but the site's theme is `data-theme` + localStorage and **defaults to light regardless of the OS**. Measured: agrees only when OS is light and the visitor has not toggled. It disagrees for every **dark-OS visitor who has not toggled** (page light, chrome dark — the default experience for that whole segment), and for anyone whose toggle differs from their OS. CSS cannot fix it; `theme-color` is a meta tag, not a styleable property. Two honest options: **(a)** drive one `theme-color` from JS — ~3 lines in the toggle plus 2 in the inline anti-flash script so the pre-paint value is right, which also replaces the two media-scoped tags with one; or **(b)** remove `theme-color` entirely and let the browser's neutral default chrome stand, which never contradicts the page. Recommend (a) if mobile polish matters, (b) if touching the anti-flash script is not worth the risk.
