...
- [ ] **`/api/contact` returns 500 on production — Resend is rejecting the send.** Narrowed by black-box probing (13 Aug 2026): the handler passes both the `RESEND_API_KEY` and `CONTACT_TO_EMAIL` guards and makes a real outbound call before failing, so **both of those env vars are present**. Evidence: warm request latency separates cleanly with no overlap — validation failure 0.584–0.615s and honeypot 0.586–0.725s return without touching the network, while a valid submission is consistently 0.667–0.709s. Stage and production both fail identically, and production's sender is the unchanged hardcoded `onboarding@resend.dev`, so this is **not** a malformed-From problem — the common factor is the API key or the Resend account state. Most likely `RESEND_API_KEY` was rotated/regenerated while verifying the domain and Vercel still holds the old value. **The exact status is in the function log (`contact api: Resend error <status>`), which needs dashboard access.** Fastest check: `curl -sS -X POST https://api.resend.com/emails -H "Authorization: Bearer $RESEND_API_KEY" -H "Content-Type: application/json" -d '{"from":"Portfolio Contact <hello@azeezdamilare.com>","to":["azeezdamilare31@gmail.com"],"subject":"Key test","html":"<p>test</p>"}'` — a 401 means the key, a 403 means the sender/recipient restriction, a 422 means the payload.
