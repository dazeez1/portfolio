...
- **DONE (15 Aug 2026): every route is pre-rendered to static HTML at build time**, via React Router framework mode (`react-router.config.ts`, `ssr: false` + `prerender`). Crawlers and link previews receive real content — title, description, canonical, OG/Twitter and JSON-LD are all in the raw HTML with JavaScript disabled. Adding a route means adding it to `src/routes.ts` **and** the prerender list; case-study paths come from `caseStudySlugs` automatically.
- **`frontend/vercel.json` has no rewrites.** It sets `outputDirectory: build/client` and `buildCommand`, and keeps the `/resume.pdf` Content-Disposition header. Deep links and hard reloads resolve because each route is a real file on disk. **Never reintroduce a catch-all rewrite to a single HTML file**: Vercel checks the filesystem before rewrites so it would not shadow most routes, but any path it did catch would serve one route's document while the client router rendered another — a hydration mismatch, not merely an SEO problem. Unmatched paths are served by `build/client/404.html` (emitted by `scripts/emit-404.mjs` from the prerendered splat route) with a real 404 status.
- Per-route `<title>`, meta description, OG + Twitter tags use **React 19 native document metadata** — plain `<title>`/`<meta>`/`<link>` elements rendered in the page component, which React hoists into `<head>`. `react-helmet-async` was removed on 14 Aug 2026. JSON-LD stays where it is rendered (React does not hoist `<script>`); Google reads it anywhere in the document.
